	...
	if ($lrequest == "Sincronizar"){
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_assoc()) {
				$lAll = $lAll . ("SYSTEMCHAT - " . $row["mensagem"] . 
								 ". - \n");
			}
			echo nl2br($lAll);
		}
		...
		if ($result->num_rows > 0) {
			...
			if ($conn->query($sql) === TRUE) { } else { echo nl2br("Resultado: \n - Falhou na Atualizacao - \n") . $conn->error; }
			$lAll = '';
			...
			('$lnome', '$lhp', '$lmp', '$lposX', '$lposY', '$lmap', '$llevel', '$lsetchar','$lhair', '$lhat', '$lweapon', '$lbattle' ,'$lside', '$lpos', '$lskillOnline','$ldata') ";
